...
- Produces: `app.partner_id` GUC; `app.current_user_id` GUC (Rev C — introduced HERE, set ONLY from the verified authenticated principal UUID, never from client input; W6 consumes it read-only); JWT claim; details map entry.

- [ ] Step 1: exact failing test — `PartnerClaimBindingTest` (`local` IT), FIVE test-case groups (Rev C): (G1) valid partner-user JWT ⇒ request context carries ALL THREE GUCs — `app.tenant_id`, `app.partner_id` (ACTIVE membership), `app.current_user_id` (authenticated user UUID) — each verifiable inside a transaction via `current_setting(..., true)`; (G2) missing/invalid/expired JWT ⇒ 401 and NO GUC set on any connection (the three-GUC context originates ONLY from verified server-side authentication — no client-header/parameter fallback, fail-closed); (G3) non-partner authenticated user ⇒ `app.tenant_id` + `app.current_user_id` set, `app.partner_id` NULL; (G4) platform/control-plane session ⇒ `app.tenant_id` = carrier, `app.partner_id` NULL, `app.current_user_id` set; (G5) suspension-then-re-auth: after suspension (session_version bumped) the user's NEWLY MINTED JWT carries NO `partner_id` claim and the fresh request context has `app.partner_id` NULL — asserted IMMEDIATELY with no sleep, proving NO 5-second membership-cache staleness window (cache key `tenant:user:session_version` rotates on the bump). `TenantRlsPartnerGucTest`: connection handler sets `app.partner_id` AND `app.current_user_id` when details carry them; FORCE-RLS partner table honors them (`current_setting('app.partner_id', true)` returns the UUID inside the transaction).
...
- [ ] Step 3: exact minimal implementation — the four modifications above; `TenantRlsConnectionHandler` additionally `SET LOCAL app.current_user_id = '<authenticated user uuid>'` beside the tenant/partner lines (sourced ONLY from `Authentication` details — never request parameters); `PartnerClaimResolver` reads `partner_id` from `Authentication.details`.
- [ ] Step 4: exact command proving GREEN — same as Step 2 → green.
...
- Modify: `partner/application/PartnerAdminService.java` (create/suspend/downgrade/transfer partner users — EVERY mutation that changes an ACTIVE membership or its role: create-ACTIVE, suspend, role downgrade, partner transfer — increments the affected user's `users.session_version` AND calls `SessionVersionCache.invalidate(tenantId, userId)` AND evicts the partner-membership claim cache entry, all in the SAME transaction)
