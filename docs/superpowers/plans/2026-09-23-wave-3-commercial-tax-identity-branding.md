...
- [ ] Step 3: exact minimal implementation — `V20260926_2__platform_files_partner_extension_rls.sql`:
  ...
  ONE fail-closed principal-aware policy: `ALTER TABLE platform_files ENABLE ROW LEVEL SECURITY; ALTER TABLE platform_files FORCE ROW LEVEL SECURITY; DROP POLICY IF EXISTS platform_files_tenant_isolation ON platform_files; CREATE POLICY principal_isolation ON platform_files FOR ALL USING ( (tenant_id IS NOT NULL AND tenant_id::text = current_setting('app.tenant_id', true) AND tenant_id::text <> '00000000-0000-0000-0000-000000000001') OR (partner_id IS NOT NULL AND partner_id::text = current_setting('app.partner_id', true) AND tenant_id::text = '00000000-0000-0000-0000-000000000001') OR (tenant_id::text = '00000000-0000-0000-0000-000000000001' AND partner_id IS NULL AND current_setting('app.tenant_id', true) = '00000000-0000-0000-0000-000000000001') ) WITH CHECK (same predicates);` — one policy, three disjoint principal branches (tenant-owned / partner-owned-carrier / platform-owned), DROP-first of the EXACT existing policy name `platform_files_tenant_isolation` (Rev C correction — the Rev B text named a non-existent `tenant_isolation`), no OR-combining with any permissive legacy policy, NULL-GUC ⇒ no branch matches ⇒ zero rows. Post-migration verification step (Rev C): `SELECT polname, polcmd, polroles, pg_get_expr(polqual, polrelid), pg_get_expr(polwithcheck, polrelid) FROM pg_policy WHERE polrelid = 'platform_files'::regclass;` executed in the test — must show EXACTLY one policy `principal_isolation` (the legacy name absent) with non-empty identical `polqual`/`polwithcheck` expressions.
...
- [ ] Step 1: exact failing test — metadata 3 MB (size > 2_097_152) rejected; `.svg` metadata with `image/png` mime rejected by magic-byte allowlist; 1 MB PNG metadata accepted and registered with `source_module='COMMERCIAL'` + `source_entity_type='BRAND_LOGO'`; partner-owned registration lands with carrier tenant + partner_id set (ambiguous-ownership CHECK satisfied); DB row + brand_profile pointer update atomic (rollback on failure leaves zero rows); re-registration replaces pointer, old file row retained; a `storage_reference` that is NULL/blank rejected (fail-closed — no fabricated reference).
