...
**Revision D (R3) changes in this wave:** Task 2 `platform_files` policy branches are written and tested EXACTLY as the three disjoint principal branches — the TENANT branch requires `partner_id IS NULL`, the PLATFORM branch requires `app.partner_id IS NULL`, and `PlatformFilesForceRlsPostgresTest` gains the explicit negative case: a partner session carrying the control-plane tenant carrier MUST NOT see platform-owned rows; the migration drops the EXACT existing policy name `platform_files_tenant_isolation` first and leaves EXACTLY ONE effective principal policy, ENABLE+FORCE; Task 9 registration-only scope is stated honestly — `LogoUploadService.registerStored(principalId, storageReference, mimeType, sizeBytes, checksumSha256)` receives no bytes and makes NO magic-byte/content-sniffing claim; validation is limited to the metadata the service actually possesses; `REAL_BYTE_STORAGE_ADAPTER` remains an explicit out-of-scope dependency blocking end-to-end multipart/content-validation claims, NOT W3 registration scope; Task 13 is restated as a PRECONDITION → VERIFY → EVIDENCE task ending with NO TRACKED COMMIT — evidence external/untracked.
...
§14.3 (Revision D) is binding: no `platform_files.kind`; ownership matrix platform/partner/tenant; ambiguous-ownership CHECK; ONE fail-closed policy; full workflow attachment regression. Agreement schemas and temporal integrity live in W4 (`V20260927_1`), not here.

## Implementation Baseline

Repository evidence at `8d0d49c7`: `platform_files` DDL is `V20260911_2__r1_attachments_external_foundation.sql` lines 17–41 — columns exactly `id, tenant_id NOT NULL, source_module VARCHAR(50) NOT NULL, source_entity_type VARCHAR(100), source_entity_id UUID, mime_type, size_bytes, checksum_sha256 CHAR(64), classification (CHECK 'PUBLIC','INTERNAL','CONFIDENTIAL','RESTRICTED'), storage_reference NOT NULL, uploaded_by, created_at` + `uk_platform_files_tenant_id UNIQUE (tenant_id, id)`; NO `kind`, NO `partner_id`. REV C CORRECTION (re-verified): the SAME migration lines 37–39 ALREADY do `ALTER TABLE platform_files ENABLE ROW LEVEL SECURITY;` + `CREATE POLICY platform_files_tenant_isolation` (fail-closed tenant-only branch, NOT FORCE, no partner/platform branches) — the table is NOT RLS-less; the existing policy name to DROP is exactly `platform_files_tenant_isolation`. Writes go through `storage/PlatformFileReferenceService.register(UUID tenantId, String sourceModule, String sourceEntityType, UUID sourceEntityId, String mimeType, Long sizeBytes, String checksumSha256, String classification, String storageReference, UUID uploadedBy)` (lines 40–58 — REGISTRATION-ONLY: caller supplies the opaque `storage_reference`; NO byte transport exists; javadoc: "no object storage / file service exists in the deployment") and `workflow/application/WorkflowAttachmentService.attach(...)` (line 45); test precedent `workflow/integration/WorkflowAttachmentExternalFoundationTest.java`. No `business_principals`/`business_*` tables exist. Control-plane tenant carrier `00000000-0000-0000-0000-000000000001` (SQL constant `control_tenant`, `V20260921_1` line 18). `btree_gist`/EXCLUDE precedent exists but is NOT needed in W3 (agreement EXCLUDE is W4).
...
4. `platform_files` RLS becomes ENABLE + FORCE with ONE fail-closed principal-aware policy; the migration first drops the EXISTING policy name `platform_files_tenant_isolation` (created by `V20260911_2` lines 37–39) and then leaves EXACTLY ONE effective principal policy verified against `pg_policy`.
...
- [ ] Step 1: exact failing test — `security/rls/PlatformFilesForceRlsPostgresTest.java`: (a) ambiguous ownership INSERT (`partner_id` set AND `tenant_id` = a customer tenant) rejected SQLState 23514 by `ck_platform_files_ownership`; (b) partner-owned row (`tenant_id` = control-plane carrier `00000000-0000-0000-0000-000000000001`, `partner_id` set) accepted; (c) tenant-owned row (`tenant_id` = actual tenant, `partner_id` NULL) accepted; (d) platform-owned row (carrier tenant, `partner_id` NULL) accepted; (e) RLS: with GUC `app.tenant_id` = customer tenant A → sees only tenant-A rows, NOT partner rows, NOT other tenants; with GUC `app.partner_id` = partner A and carrier tenant context → sees only partner-A asset rows; with NULL GUCs → ZERO rows (fail-closed FORCE — even table owner); (e2) EXPLICIT NEGATIVE (Revision D): a partner session carrying the control-plane tenant carrier (`app.tenant_id` = carrier AND `app.partner_id` = partner A) MUST NOT see PLATFORM-owned rows (rows with carrier tenant AND `partner_id` NULL) — the PLATFORM branch requires `app.partner_id IS NULL` and the assertion proves the carrier context cannot read platform-owned assets; (f) `platform_files.kind` does NOT exist (`information_schema.columns` assertion); (g) `pg_policy` verification: after migration EXACTLY one policy `principal_isolation` exists (legacy name `platform_files_tenant_isolation` absent) with non-empty identical `polqual`/`polwithcheck` and `relforcerowlevelsecurity = true`.
- [ ] Step 2: exact command proving RED — `cd apps/sanad-platform && mvn -B -ntp -Dsurefire.useFile=false -Dtest=com.sanad.platform.security.rls.PlatformFilesForceRlsPostgresTest test` → `partner_id` column missing; and AS TABLE OWNER a NULL-GUC read returns rows (RLS is ENABLE but NOT FORCE today — the owner bypasses the existing tenant-only policy) and the policy register has no partner/platform branches ⇒ red. (Rev C baseline correction: RLS is NOT absent — `V20260911_2` already ENABLEd it with fail-closed policy `platform_files_tenant_isolation`; the red conditions are the missing FORCE and the missing principal branches.)
- [ ] Step 3: exact minimal implementation — `V20260926_2__platform_files_partner_extension_rls.sql`:
  ...
  ONE fail-closed principal-aware policy with THREE DISJOINT PRINCIPAL BRANCHES written exactly as follows (Revision D): `ALTER TABLE platform_files ENABLE ROW LEVEL SECURITY; ALTER TABLE platform_files FORCE ROW LEVEL SECURITY; DROP POLICY IF EXISTS platform_files_tenant_isolation ON platform_files; CREATE POLICY principal_isolation ON platform_files FOR ALL USING ( -- TENANT branch: tenant_id = app.tenant_id AND tenant_id != CONTROL_PLANE_TENANT AND partner_id IS NULL (tenant_id IS NOT NULL AND tenant_id::text = current_setting('app.tenant_id', true) AND tenant_id::text <> '00000000-0000-0000-0000-000000000001' AND partner_id IS NULL) OR -- PARTNER branch: tenant_id = CONTROL_PLANE_TENANT AND partner_id IS NOT NULL AND partner_id = app.partner_id (tenant_id::text = '00000000-0000-0000-0000-000000000001' AND partner_id IS NOT NULL AND partner_id::text = current_setting('app.partner_id', true)) OR -- PLATFORM branch: tenant_id = CONTROL_PLANE_TENANT AND partner_id IS NULL AND app.tenant_id = CONTROL_PLANE_TENANT AND app.partner_id IS NULL (tenant_id::text = '00000000-0000-0000-0000-000000000001' AND partner_id IS NULL AND current_setting('app.tenant_id', true) = '00000000-0000-0000-0000-000000000001' AND current_setting('app.partner_id', true) IS NULL) ) WITH CHECK (identical predicates);` — the TENANT branch requires `partner_id IS NULL` (a partner-owned row is never tenant-visible even if the tenant id collided) and the PLATFORM branch requires BOTH the carrier tenant GUC AND `app.partner_id` NULL, so a partner session carrying the control-plane tenant carrier can NEVER see platform-owned rows; three disjoint principal branches (tenant-owned / partner-owned-carrier / platform-owned), DROP-first of the EXACT existing policy name `platform_files_tenant_isolation` (Rev C correction — the Rev B text named a non-existent `tenant_isolation`), no OR-combining with any permissive legacy policy, NULL-GUC ⇒ no branch matches ⇒ zero rows; EXACTLY ONE effective principal policy remains, ENABLE+FORCE. Post-migration verification step (Rev C): `SELECT polname, polcmd, polroles, pg_get_expr(polqual, polrelid), pg_get_expr(polwithcheck, polrelid) FROM pg_policy WHERE polrelid = 'platform_files'::regclass;` executed in the test — must show EXACTLY one policy `principal_isolation` (the legacy name absent) with non-empty identical `polqual`/`polwithcheck` expressions.
...
- Produces: `invoice_party_snapshots` (FORCE RLS composite; `invoice_id NOT NULL` bound at capture; FK to `billing_invoices(id)` shipped HERE — `billing_invoices` exists since `V19`, no forward reference is needed; Rev C lifecycle: the invoice UUID is allocated FIRST, then SELLER and BUYER snapshots are INSERTed already bound — post-issuance late-binding UPDATE is forbidden).

- [ ] Step 1: exact failing test — `commercial/domain/InvoiceSnapshotSchemaPostgresTest.java`: table exists with §15 fields; `snapshot_for` CHECK `('SELLER','BUYER')`; `invoice_id uuid NOT NULL REFERENCES billing_invoices(id)` (FK enforced — random-UUID insert rejected 23503; Rev C correction: NOT nullable, no W4 forward reference); `uq_snapshot_per_invoice_side UNIQUE (invoice_id, snapshot_for)`; denormalized tenant/partner scope columns with the same CHECK discipline; composite FORCE RLS isolation.
...
- [ ] Step 3: exact minimal implementation — `V20260926_3__invoice_party_snapshots.sql`: `invoice_party_snapshots(id uuid pk, snapshot_for text NOT NULL CHECK (snapshot_for IN ('SELLER','BUYER')), principal_id uuid NOT NULL REFERENCES business_principals(id), invoice_id uuid NOT NULL REFERENCES billing_invoices(id), legal_name text NOT NULL, trade_name text, commercial_registration_number text, tax_registration_number text, country char(2), billing_address jsonb NOT NULL, logo_file_id uuid NULL, currency_code char(3) NOT NULL, captured_at timestamptz NOT NULL DEFAULT now(), captured_by uuid NOT NULL, tenant_id uuid NULL, partner_id uuid NULL, <type-discipline CHECKs>)` + `CREATE UNIQUE INDEX uq_snapshot_per_invoice_side ON invoice_party_snapshots (invoice_id, snapshot_for)` + `CREATE INDEX idx_snapshot_invoice ON invoice_party_snapshots (invoice_id)` + composite FORCE RLS. Header comment documents the Rev C capture ORDER: allocate invoice UUID → INSERT SELLER snapshot (bound) → INSERT BUYER snapshot (bound) → issue (snapshots immediately immutable via Task 5 trigger); late-binding UPDATE of `invoice_id` is structurally impossible (NOT NULL + trigger).
- [ ] Step 4: exact command proving GREEN — same as Step 2 → green.
...
### Task 9: Logo registration via real platform_files (source_module='COMMERCIAL' — registration of an already-stored object)

Files:
- Create: `commercial/application/StorageAdapter.java` (port interface — `String store(byte[] bytes, String suggestedMimeType)` returning the opaque `storage_reference`; contract only in W3), `commercial/application/LogoUploadService.java`
...
Interfaces:
- Consumes: `platform_files` with `source_module='COMMERCIAL'`, `source_entity_type='BRAND_LOGO'`, `source_entity_id=<business principal id>`; NO `kind` field (spec §14.3). REV C SCOPE LIMIT (`W3_STORAGE_ADAPTER=BLOCKED` for real byte transport): the repository has NO byte-storage adapter — `PlatformFileReferenceService` is registration-only and its javadoc records that no object/file storage service exists. W3 therefore delivers (a) the `StorageAdapter` PORT CONTRACT, and (b) `LogoUploadService.registerStored(principalId, storageReference, mimeType, sizeBytes, checksumSha256)` — registration of an ALREADY-STORED object whose `storage_reference` is caller-supplied. REVISION D VALIDATION HONESTY: `registerStored` receives NO bytes and therefore makes NO claim to validate PNG/JPEG magic bytes or to detect an SVG whose caller lies about MIME; validation is limited to the metadata the service actually possesses: `storageReference` non-null/non-blank; declared MIME allowlist `image/png` and `image/jpeg` ONLY; `sizeBytes > 0 && sizeBytes <= 2_097_152`; lowercase/normalized SHA-256 format of exactly 64 hex chars; ownership/principal congruence and FORCE-RLS context; atomic `platform_files` row + `brand_profiles` pointer transaction. Real magic-byte/content verification belongs to the future trusted byte-storage adapter/read path; `REAL_BYTE_STORAGE_ADAPTER` remains an explicit out-of-scope dependency that blocks any end-to-end multipart/content-validation claim but does NOT block W3 registration scope. A browser-multipart `upload(principalId, multipart)` endpoint REQUIRING real byte storage is NOT delivered by W3 and is recorded as that explicitly BLOCKED out-of-scope dependency pending a future storage-infrastructure decision — no adapter is invented and no fake local-disk store is added.
- Produces: `registerStored(...)` — atomic transaction: `platform_files` INSERT (via `register`/`registerForPartner`) + `brand_profiles.primary_logo_file_id`/`invoice_logo_file_id` pointer update (new `brand_profiles` table in `V20260926_2` — `id uuid pk, principal_id uuid NOT NULL UNIQUE REFERENCES business_principals(id), primary_logo_file_id uuid NULL, invoice_logo_file_id uuid NULL, theme_accent text NULL, timestamps, version bigint NOT NULL DEFAULT 0`, composite FORCE RLS) commit together or roll back together — a failed DB commit leaves NO platform_files row (and, because the object was already stored externally, no orphan to clean; the `StorageAdapter.store` path, when it exists later, MUST define its own orphan-cleanup callback invoked on DB-commit failure — recorded in the port javadoc); re-registration replaces the pointer, the old file row retained (history); authorized GET read-back returns METADATA + the opaque reference only (no public bucket, no byte proxy in W3).

- [ ] Step 1: exact failing test — declared `image/svg+xml` MIME rejected (declared MIME allowlist — NO content sniffing is asserted or claimed); metadata 3 MB (size > 2_097_152) rejected (oversized metadata); 1 MB PNG-declared metadata with conforming 64-hex checksum accepted and registered with `source_module='COMMERCIAL'` + `source_entity_type='BRAND_LOGO'`; blank/NULL `storageReference` rejected (fail-closed — no fabricated reference); malformed checksum (not exactly 64 lowercase hex chars) rejected; cross-principal registration (principal congruence violation) rejected; partner-owned registration lands with carrier tenant + partner_id set (ambiguous-ownership CHECK satisfied); DB row + brand_profile pointer update atomic (rollback on failure leaves zero rows); re-registration replaces pointer, old file row retained.
...
- [ ] Step 3: exact minimal implementation — `StorageAdapter` port + `LogoUploadService.registerStored(...)` + `brand_profiles` DDL appended to `V20260926_2__platform_files_partner_extension_rls.sql` (same migration, FORCE RLS composite).
- [ ] Step 4: exact command proving GREEN — same as Step 2 → green.
...
- [ ] Step 6: exact commit — `git commit -m "wave3(branding): logo registration via source_module=COMMERCIAL, no kind column, storage port contract (C3)"`.
...
- Produces: `captureForInvoice(UUID invoiceId, UUID principalId, String snapshotFor)` (Rev C signature — invoice-bound capture replacing the Rev B `capture(principalId, snapshotFor)`) — CALLER ALLOCATES the invoice UUID first (insert or pre-allocated id inside the issuance transaction); this service renders ALL §15 fields into a NEW immutable row INSERTed already bound to that `invoice_id` (NOT NULL — Rev C W3 Task 4); per call one row (history, never update); consumed by W4 issuance in the order DRAFT row → SELLER snapshot → BUYER snapshot → issue; NO post-issuance binding UPDATE exists (structurally impossible: NOT NULL + immutability trigger).

- [ ] Step 1: exact failing test — captureForInvoice renders legal/trade identity, tax identity, billing address jsonb, logo pointer, currency bound to the passed invoice id; second capture for same principal+invoice = second row only for the OTHER side (unique `(invoice_id, snapshot_for)` blocks duplicates of one side); any subsequent UPDATE/DELETE attempt on the row hits the immutability trigger; unbound capture (no invoice id) is a compile-time impossibility (signature change).
...
### Task 13: Wave exit evidence battery (verification/evidence task — PRECONDITION → VERIFY → EVIDENCE)
...
- [ ] Step 1: PRECONDITION — the FINAL_WAVE_SHA candidate exists, `git status --short` is clean, and every W3 implementation task is committed green; battery environment up.
...
**Dependencies:** W2 (partner context, delegation gate). Blocks W4 (snapshots + agreement identity). **Security:** profiles tenant/partner-jailed (composite FORCE RLS); principal uniqueness DB-enforced (partial indexes); snapshots physically immutable (trigger); `platform_files` FORCE hardening closes an unguarded shared table with ONE non-overbroad three-branch principal policy (TENANT branch requires `partner_id IS NULL`; PLATFORM branch requires `app.partner_id IS NULL` — a partner carrier session never sees platform-owned rows); logo registration validates ONLY the metadata the service possesses (declared MIME allowlist, size cap, checksum format, principal congruence) and makes NO magic-byte/content-sniffing claim — real content verification belongs to the future trusted byte-storage adapter (`REAL_BYTE_STORAGE_ADAPTER` stays an explicit out-of-scope dependency); authorized reads only; no legal/tax rules hard-coded (§33). **Rollback:** flag `SANAD_COMMERCIAL_IDENTITY_ENABLED=false` ⇒ controllers 404-guarded and W4 issuance refuses snapshot capture; DDL additive; the platform_files RLS hardening is forward-only security improvement with a documented owner-held compensating script (re-creating the previous no-policy state) for emergency only; trigger removal script documented in the migration header (owner-run, never shipped as auto-down); revert commits safe.
