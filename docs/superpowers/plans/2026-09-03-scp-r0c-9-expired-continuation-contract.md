...
## Governance Freeze — R0C-9C

This section supersedes the earlier credential/durability caveat as current
closure state. The original R0C-9 chain ending at
`75b757f2c5902d60956adab5a8b32276b4f41349` has since been recovered from
the verified bundle and pushed durably to
`origin/scp/r0c-9-expired-continuation-contract`.

```text
SUBSCRIPTION_MULTIPLICITY_MODEL = MODEL_B

HISTORICAL_MULTIPLE_SUBSCRIPTIONS_ALLOWED = YES
TERMINAL_HISTORY_CARDINALITY = 0..N

MULTIPLE_SIMULTANEOUS_EFFECTIVE_SUBSCRIPTIONS_ALLOWED = NO
EFFECTIVE_SUBSCRIPTION_CARDINALITY = 0..1_PER_TENANT
EFFECTIVE_SUBSCRIPTION_RULE = UNIQUE_NON_TERMINAL

TERMINAL_STATUSES = CANCELLED,EXPIRED,TERMINATED

EXPIRED_CONTINUATION = CREATE_NEW_SUBSCRIPTION_ROW
EXPIRED_REACTIVATION = FORBIDDEN
OLD_EXPIRED_ROW_MUTATED = NO

CANCELLED_CONTINUATION = RESUME_EXISTING_ROW
CREATE_NEW_AFTER_CANCELLED = REJECT
RESUME_CANCELLED_WHILE_SUCCESSOR_EXISTS = REJECT

TERMINATED_CONTINUATION = DEFERRED_PRODUCT_DECISION

AUTOMATIC_SECOND_TRIAL_AFTER_EXPIRED = NO
EXPLICIT_REPEAT_TRIAL = OUT_OF_SCOPE

BILLING_EFFECTIVE_SUBSCRIPTION = UNIQUE_NON_TERMINAL
HISTORICAL_INVOICE_AUTHORITY = subscription_id
HISTORICAL_OVERDUE_INVOICE_MAY_DUN_SUCCESSOR = NO

ENTITLEMENT_EFFECTIVE_SUBSCRIPTION = UNIQUE_NON_TERMINAL
PROVISIONING_AUTHORITY = subscription_id

HISTORY_READ_CARDINALITY = 0..N
CURRENT_EFFECTIVE_READ_CARDINALITY = 0..1

LEGACY_EXPIRED_RESUME_FALSE_SUCCESS = P1_OPEN_FOR_R0C10
...
OLD_BINARY_PLUS_MULTIPLE_ROWS = FORBIDDEN
