...
export default function UsersLivePanel() {
  ...
  const [notice, setNotice] = useState("أدخل Tenant UUID لتحميل المستخدمين الفعليين.");
  ...
  async function loadUsers() {
    if (!tenantIsValid) {
      setError({ title: "UUID غير صالح", message: "Tenant UUID غير صالح.", kind: "validation" });
      return;
    }
    setBusy("load");
    setError(null);
    try {
      const result = await usersApi.list(normalizedTenantId);
      setUsers(result);
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const saved = await usersApi.create(normalizedTenantId, {
        email,
  ...
  async function transition(item: UserResponse, action: UserLifecycleAction) {
    ...
    setError(null);
    try {
      const saved = await usersApi.transition(normalizedTenantId, item.id, action);
      ...
      <div className="rounded-3xl border border-teal-900/10 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-500">يُمرر Tenant UUID صراحة إلى Backend. لا توجد مصادقة أو هوية مستخدم تلقائية في هذه المرحلة.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
          ...
          <button type="button" onClick={loadUsers} disabled={busy !== null} className="self-end rounded-xl bg-teal-800 px-5 py-2.5 font-black text-white disabled:opacity-50">
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
          <form onSubmit={createUser} className="grid content-start gap-3 rounded-2xl border border-slate-200 p-4">
            ...
            <button disabled={busy !== null || !tenantIsValid} className="rounded-xl bg-teal-800 px-4 py-2.5 font-black text-white disabled:opacity-50">
              {busy === "create" ? "جارٍ الحفظ…" : "إنشاء في Backend"}
            </button>
          </form>
                ...
                <article key={item.id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    {item.status !== "SUSPENDED" && (
                      <button type="button" disabled={busy !== null} onClick={() => transition(item, "suspend")} className="rounded-lg border border-rose-200 px-3 py-1.5 text-xs font-black text-rose-700 disabled:opacity-50">إيقاف</button>
                    )}
                    {item.status !== "ARCHIVED" && (
                      <button type="button" disabled={busy !== null} onClick={() => transition(item, "archive")} className="rounded-lg border border-violet-200 px-3 py-1.5 text-xs font-black text-violet-700 disabled:opacity-50">أرشفة</button>
                    )}
                  </div>
                </article>
              ))
