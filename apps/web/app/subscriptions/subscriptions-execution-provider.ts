/**
 * Subscriptions Execution Provider
 * ---------------------------------
 * Implements the ExecutionProvider interface for the Subscriptions module.
...
import {
  SUBSCRIPTIONS_GROUP_DATA,
  SUBSCRIPTIONS_TASKS,
  type SubscriptionsTask,
} from "./subscriptions-execution-data";

/**
 * Subscriptions Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Subscriptions module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class SubscriptionsExecutionProvider {
  readonly moduleId = "SUBSCRIPTIONS";
  readonly moduleName = "Subscription Management";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = SUBSCRIPTIONS_GROUP_DATA.map((groupData) =>
      this.buildGroup(groupData)
    );
    ...
    return {
      id: "SUBSCRIPTIONS-PROGRAM",
      code: "SUBSCRIPTIONS",
      titleAr: "نظام إدارة الاشتراكات",
      titleEn: "Subscription Management",
      descriptionAr:
        "نظام شامل لإدارة الاشتراكات وخطط الأسعار والفوترة وبوابة العميل والتحليلات",
      descriptionEn:
        "Comprehensive system for managing subscriptions, pricing plans, billing, customer portal, and analytics",
      status,
      groups,
    };
  }
  ...
  private buildGroup(groupData: {
    ...
    const tasks = SUBSCRIPTIONS_TASKS.filter(
      (t) => t.groupCode === groupData.code
  ...
  private buildTask(taskData: SubscriptionsTask): ExecutionTask {
    return {
      id: taskData.id,
