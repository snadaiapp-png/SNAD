/**
 * Licensing Execution Provider
 * ----------------------------
 * Implements the ExecutionProvider interface for the Licensing module.
...
import { LICENSING_GROUP_DATA, LICENSING_TASKS, type LicensingTask } from "./licensing-execution-data";

/**
 * Licensing Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Licensing module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class LicensingExecutionProvider {
  readonly moduleId = "LICENSING";
  readonly moduleName = "Licensing Management";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = LICENSING_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      id: "LICENSING-PROGRAM",
      code: "LICENSING",
      titleAr: "إدارة التراخيص",
      titleEn: "Licensing Management",
      descriptionAr: "نظام شامل لإدارة التراخيص وتوليد المفاتيح والتحقق ودورة حياة الترخيص",
      descriptionEn: "Comprehensive system for license management, key generation, validation, and lifecycle",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: LicensingTask): ExecutionTask {
    return {
      id: taskData.id,
