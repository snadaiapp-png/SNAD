 ...
 * ----------------------
 * Implements the ExecutionProvider interface for the CRM module.
...
import { CRM_GROUP_DATA, CRM_TASKS, type CrmTask } from "./crm-execution-data";

/**
 * CRM Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the CRM module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class CrmExecutionProvider {
  readonly moduleId = "CRM";
  readonly moduleName = "Customer Relationship Management";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = CRM_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
  ...
  private buildTask(taskData: CrmTask): ExecutionTask {
    return {
      id: taskData.id,
