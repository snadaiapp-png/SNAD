/**
 * Identity Execution Provider
 * ---------------------------
 * Implements the ExecutionProvider interface for the Identity & Access Management module.
...
import { IDENTITY_GROUP_DATA, IDENTITY_TASKS, type IdentityTask } from "./identity-execution-data";

/**
 * Identity Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Identity & Access Management module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class IdentityExecutionProvider {
  readonly moduleId = "IDENTITY";
  readonly moduleName = "Identity & Access Management";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = IDENTITY_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      id: "IDENTITY-PROGRAM",
      code: "IDENTITY",
      titleAr: "نظام إدارة الهوية والصلاحيات",
      titleEn: "Identity & Access Management",
      descriptionAr: "نظام شامل لإدارة المستخدمين والمصادقة والتفويض والامتثال الأمني",
      descriptionEn: "Comprehensive system for user management, authentication, authorization, and security compliance",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: IdentityTask): ExecutionTask {
    return {
      id: taskData.id,
