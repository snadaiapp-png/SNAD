/**
 * ERP Execution Provider
 * ----------------------
 * Implements the ExecutionProvider interface for the ERP module.
...
import { ERP_GROUP_DATA, ERP_TASKS, type ErpTask } from "./erp-execution-data";

/**
 * ERP Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the ERP module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class ErpExecutionProvider {
  ...
  private buildProgram(): ExecutionProgram {
    const groups = ERP_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      id: "ERP-PROGRAM",
      code: "ERP",
      titleAr: "نظام تخطيط موارد المؤسسات",
      titleEn: "Enterprise Resource Planning",
      descriptionAr: "نظام ERP شامل لإدارة المشتريات والمبيعات والمخزون والتصنيع والمحاسبة",
      descriptionEn: "Comprehensive ERP system for procurement, sales, inventory, manufacturing, and accounting",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: ErpTask): ExecutionTask {
    return {
      id: taskData.id,
