/**
 * AI Platform Execution Provider
 * ------------------------------
 * Implements the ExecutionProvider interface for the AI Platform module.
...
import { AI_PLATFORM_GROUP_DATA, AI_PLATFORM_TASKS, type AiPlatformTask } from "./ai-platform-execution-data";

/**
 * AI Platform Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the AI Platform module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class AiPlatformExecutionProvider {
  readonly moduleId = "AI_PLATFORM";
  readonly moduleName = "AI Platform";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = AI_PLATFORM_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      id: "AI_PLATFORM-PROGRAM",
      code: "AI_PLATFORM",
      titleAr: "منصة الذكاء الاصطناعي",
      titleEn: "AI Platform",
      descriptionAr: "منصة متكاملة للذكاء الاصطناعي تشمل تدريب النماذج ونشرها وتحليل البيانات والتوقعات",
      descriptionEn: "Comprehensive AI platform covering model training, deployment, data analytics, and predictions",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: AiPlatformTask): ExecutionTask {
    return {
      id: taskData.id,
