 ...
 * ----------------------------
 * Implements the ExecutionProvider interface for the Analytics module.
...
import { ANALYTICS_GROUP_DATA, ANALYTICS_TASKS, type AnalyticsTask } from "./analytics-execution-data";

/**
 * Analytics Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Analytics module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class AnalyticsExecutionProvider {
  readonly moduleId = "ANALYTICS";
  readonly moduleName = "Analytics & Business Intelligence";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = ANALYTICS_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      ...
      code: "ANALYTICS",
      titleAr: "نظام التحليلات وذكاء الأعمال",
      titleEn: "Analytics & Business Intelligence",
      descriptionAr: "نظام شامل للتحليلات وذكاء الأعمال يدعم التوقعات والاتجاهات وتصور البيانات",
      descriptionEn: "Comprehensive analytics and business intelligence system supporting forecasting, trends, and data visualization",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: AnalyticsTask): ExecutionTask {
    return {
      id: taskData.id,
