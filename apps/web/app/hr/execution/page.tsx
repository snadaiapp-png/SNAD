...
/**
 * HR Execution Dashboard (moved from /hr — WS5 Task 8 Step 1).
 *
 * The original execution dashboard is preserved here unchanged in behavior:
 * same execution data (hr-execution-data.ts), same provider, same progress
 * rendering. Only the route and import depth changed.
 */
export default function HrExecutionPage() {
  const { state } = useAuth();
  ...
  if (state !== "AUTHENTICATED") {
    router.replace("/?returnUrl=%2Fhr%2Fexecution");
