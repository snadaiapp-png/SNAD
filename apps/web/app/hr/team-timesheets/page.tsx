...
export default function TeamTimesheetsPage() {
  ...
  const columns: HrColumn<Timesheet>[] = [
    ...
    { key: "actions", header: t("hrm.timesheets.actions"), render: (r) => <span className={styles.actionRow}>{r.state === "SUBMITTED" ? <><button type="button" className={styles.linkButton} onClick={() => void approve(r.id)} disabled={busy}>{t("hrm.timesheets.team.approve")}</button><button type="button" className={styles.linkButton} onClick={() => void reject(r.id)} disabled={busy}>{t("hrm.timesheets.team.reject")}</button></> : null}</span> },
  ];
  const periodCount = new Set(timesheets.map((item) => `${item.periodStart}:${item.periodEnd}`)).size;

  return (
    <HrWorkspace capabilities={capabilities} activeHref="/hr/team-timesheets" translate={t}>
      <HrG2ProductSurface eyebrow={t("hrm.g2.landing.myTeam")} title={t("hrm.timesheets.team.title")} subtitle={t("hrm.timesheets.team.subtitle")}
        metrics={[
          { label: t("hrm.timesheets.team.caption"), value: timesheets.length },
          { label: t("hrm.timesheets.state"), value: timesheets.filter((item) => item.state === "SUBMITTED").length },
          { label: t("hrm.timesheets.period"), value: periodCount },
        ]}
      >
        {notice ? <p role="status" className={styles.kpiHint}>{notice}</p> : null}
        <section className={visualStyles.productPanel} data-testid="team-timesheet-queue">
          {loading ? <HrLoading /> : error ? <HrErrorState error={error} onRetry={load} /> : <div data-testid="team-timesheets-ready"><HrDataTable<Timesheet> caption={t("hrm.timesheets.team.caption")} columns={columns} rows={timesheets} rowKey={(r) => r.id} emptyTitle={t("hrm.timesheets.team.empty")} /></div>}
        </section>
      </HrG2ProductSurface>
    </HrWorkspace>
  );
