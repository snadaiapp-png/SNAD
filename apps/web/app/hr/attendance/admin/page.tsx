...
function formatTime(iso: string | null, locale: string): string { if (!iso) return "—"; return new Date(iso).toLocaleTimeString(locale === "ar" ? "ar-SA" : "en-US", { hour: "2-digit", minute: "2-digit" }); }
...
export default function AttendanceAdminPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(null); try { setRecords(await hrG2Api.listAdminAttendance()); } catch (err) { setError(err); } finally { setLoading(false); } }, []);
  ...
  const openCount = records.filter((record) => record.state === "OPEN").length;
  const closedCount = records.filter((record) => record.clockOut != null).length;
  const missingPunchCount = records.filter((record) => record.clockIn == null || (record.state !== "OPEN" && record.clockOut == null)).length;

  return (
    <HrWorkspace capabilities={capabilities} activeHref="/hr/attendance/admin" translate={t}>
      <HrG2ProductSurface eyebrow={t("hrm.g2.landing.hrOperations")} title={t("hrm.attendanceAdmin.title")} subtitle={t("hrm.attendanceAdmin.subtitle")}
        metrics={[
          { label: t("hrm.attendanceAdmin.state"), value: openCount, hint: t("hrm.attendance.state.OPEN") },
          { label: t("hrm.attendanceAdmin.clockOut"), value: closedCount },
          { label: t("hrm.attendanceReport.missingPunch"), value: missingPunchCount },
        ]}
      >
        {canCorrect ? <p role="status" className={styles.kpiHint}>{t("hrm.attendanceAdmin.correctionUnavailable")}</p> : null}
        <section className={visualStyles.productPanel} data-testid="attendance-admin-dashboard">
          {loading ? <HrLoading /> : error ? <HrErrorState error={error} onRetry={load} /> : <div data-testid="attendance-admin-ready"><HrDataTable<AttendanceRecord> caption={t("hrm.attendanceAdmin.caption")} columns={columns} rows={records} rowKey={(r) => r.id} emptyTitle={t("hrm.attendanceAdmin.empty")} /></div>}
        </section>
      </HrG2ProductSurface>
    </HrWorkspace>
  );
