...
export default function LeavePoliciesPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(null); try { setPolicies(await hrG2Api.listAdminLeaveTypes() as LeaveType[]); } catch (err) { setError(err); } finally { setLoading(false); } }, []);
  ...
  return (
    <HrWorkspace capabilities={capabilities} activeHref="/hr/leave/policies" translate={t}>
      <HrG2ProductSurface eyebrow={t("hrm.g2.landing.hrOperations")} title={t("hrm.leavePolicies.title")} subtitle={t("hrm.leavePolicies.subtitle")}
        metrics={[
          { label: t("hrm.leavePolicies.caption"), value: policies.length },
          { label: t("hrm.leavePolicies.paid"), value: paidCount },
          { label: t("hrm.leavePolicies.attachment"), value: attachmentCount },
        ]}
      >
        <section className={visualStyles.productPanel} data-testid="leave-policy-operations">
          {loading ? <HrLoading /> : error ? <HrErrorState error={error} onRetry={load} /> : <div data-testid="leave-policies-ready"><HrDataTable<LeaveType> caption={t("hrm.leavePolicies.caption")} columns={columns} rows={policies} rowKey={(r) => r.id} emptyTitle={t("hrm.leavePolicies.empty")} /></div>}
        </section>
      </HrG2ProductSurface>
    </HrWorkspace>
  );
