/**
 * Notifications Execution Provider
 * --------------------------------
 * Implements the ExecutionProvider interface for the Notifications module.
...
import { NOTIFICATIONS_GROUP_DATA, NOTIFICATIONS_TASKS, type NotificationsTask } from "./notifications-execution-data";

/**
 * Notifications Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Notifications module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class NotificationsExecutionProvider {
  readonly moduleId = "NOTIFICATIONS";
  readonly moduleName = "Notifications System";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = NOTIFICATIONS_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      id: "NOTIFICATIONS-PROGRAM",
      code: "NOTIFICATIONS",
      titleAr: "نظام الإشعارات",
      titleEn: "Notifications System",
      descriptionAr: "نظام إشعارات شامل يدعم البريد الإلكتروني والرسائل النصية والإشعارات الفورية مع قوالب وجدولة",
      descriptionEn: "Comprehensive notification system supporting email, SMS, and push notifications with templates and scheduling",
      status,
      groups,
    };
  }
  ...
  private buildGroup(groupData: {
    ...
    const tasks = NOTIFICATIONS_TASKS
  ...
  private buildTask(taskData: NotificationsTask): ExecutionTask {
    return {
      id: taskData.id,
