/**
 * Inventory Execution Provider
 * ----------------------------
 * Implements the ExecutionProvider interface for the Inventory module.
...
import { INVENTORY_GROUP_DATA, INVENTORY_TASKS, type InventoryTask } from "./inventory-execution-data";

/**
 * Inventory Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Inventory module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class InventoryExecutionProvider {
  readonly moduleId = "INVENTORY";
  readonly moduleName = "Inventory Management";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = INVENTORY_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      ...
      descriptionAr: "نظام شامل لإدارة المخزون يتضمن الأصناف، المستودعات، الاستلام، الشحن، الجرد، والتحليلات",
      descriptionEn: "Comprehensive inventory management system covering items, warehouses, receiving, shipping, cycle counting, and analytics",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: InventoryTask): ExecutionTask {
    return {
      id: taskData.id,
