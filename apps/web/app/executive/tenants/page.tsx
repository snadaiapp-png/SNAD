...
export default function TenantsPage() {
  ...
  async function handleTenantLoginLink(tenantId: string, action: "OPEN" | "COPY") {
    const popup = action === "OPEN" ? window.open("about:blank", "_blank", "noopener,noreferrer") : null;
                  ...
                  return (
                    <tr key={tenant.id}>
                      ...
                      <td data-label={t("scp.common.actions")}>
                        <div className={styles.filters}>
                          <Link href={`/executive/subscriptions?tenantId=${tenant.id}`}>{t("scp.tenants.viewSubscriptions")}</Link>
                          {canUseLoginLink ? <>
                            <Button type="button" variant="secondary" size="sm" disabled={busy} onClick={() => void handleTenantLoginLink(tenant.id, "OPEN")}>{t("scp.tenants.openLogin")}</Button>
                            <Button type="button" variant="secondary" size="sm" disabled={busy} onClick={() => void handleTenantLoginLink(tenant.id, "COPY")}>{t("scp.tenants.copyLogin")}</Button>
                          </> : null}
                          {canManage && !archived ? <>
                            <Button type="button" variant="secondary" size="sm" disabled={busy} onClick={() => void openEdit(tenant.id)}>{t("scp.tenants.update")}</Button>
                            {canFreeze ? <Button type="button" variant="secondary" size="sm" onClick={() => { setDialogError(""); setReason(""); setDialog({ kind: "status", tenantId: tenant.id, targetStatus: "SUSPENDED" }); }}>{t("scp.tenants.freeze")}</Button> : null}
                            {canReactivate ? <Button type="button" variant="secondary" size="sm" onClick={() => { setDialogError(""); setReason(""); setDialog({ kind: "status", tenantId: tenant.id, targetStatus: "ACTIVE" }); }}>{t("scp.tenants.reactivate")}</Button> : null}
                            <Button type="button" variant="danger" size="sm" onClick={() => { setDialogError(""); setReason(""); setDialog({ kind: "status", tenantId: tenant.id, targetStatus: "ARCHIVED" }); }}>{t("scp.tenants.archive")}</Button>
