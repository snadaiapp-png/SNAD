/**
 * Finance Execution Provider
 * --------------------------
 * Implements the ExecutionProvider interface for the Finance module.
...
import { FINANCE_GROUP_DATA, FINANCE_TASKS, type FinanceTask } from "./finance-execution-data";

/**
 * Finance Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the Finance module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class FinanceExecutionProvider {
  readonly moduleId = "FINANCE";
  readonly moduleName = "Financial Management";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = FINANCE_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      ...
      descriptionAr: "نظام مالي شامل لإدارة المحاسبة والمدفوعات والتحصيل والتقارير المالية",
      descriptionEn: "Comprehensive financial system for accounting, payments, collections, and financial reporting",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: FinanceTask): ExecutionTask {
    return {
      id: taskData.id,
