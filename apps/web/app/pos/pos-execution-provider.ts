/**
 * POS Execution Provider
 * ----------------------
 * Implements the ExecutionProvider interface for the POS module.
...
import { POS_GROUP_DATA, POS_TASKS, type PosTask } from "./pos-execution-data";

/**
 * POS Execution Provider
 *
 * Satisfies the ExecutionProvider interface for the POS module.
 * All execution logic uses the shared framework calculators and validators.
 */
export class PosExecutionProvider {
  readonly moduleId = "POS";
  readonly moduleName = "Point of Sale";
  ...
  private buildProgram(): ExecutionProgram {
    const groups = POS_GROUP_DATA.map((groupData) => this.buildGroup(groupData));
    ...
    return {
      ...
      descriptionAr: "نظام نقطة البيع الشامل للمتاجر",
      descriptionEn: "Comprehensive point of sale system for retail stores",
      status,
      groups,
    };
  }
  ...
  private buildTask(taskData: PosTask): ExecutionTask {
    return {
      id: taskData.id,
