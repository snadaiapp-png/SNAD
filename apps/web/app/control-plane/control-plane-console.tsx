...
export function ControlPlaneConsole() {
  ...
  useEffect(() => {
    ...
    platformOperationsApi.organizations(tenantId).then(async (orgs) => {
      ...
      // If organizations fail to load, it may be because the table is empty
      // for this tenant (not an error — just no companies yet).
  ...
  const addTenant = () => {
    const name = ask("اسم المستأجر (باللغة العربية أو الإنجليزية)");
