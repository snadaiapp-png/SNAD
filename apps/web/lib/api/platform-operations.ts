...
export const platformOperationsApi = {
  ...
  changeTenantStatus: (tenantId: string, status: string, reason: string) => apiClient.patch<ManagedTenant>(`/api/v1/control-plane/tenants/${tenantId}/status`, { status, reason }),
  ...
  createPlan: (body: PlanInput & { code: string }) => apiClient.post<SaasPlan>("/api/v1/control-plane/plans", body),
  updatePlan: (planId: string, body: PlanInput) => apiClient.put<SaasPlan>(`/api/v1/control-plane/plans/${planId}`, body),
  changePlanStatus: (planId: string, status: string, reason: string) => apiClient.patch<SaasPlan>(`/api/v1/control-plane/plans/${planId}/status`, { status, reason }),

  subscriptions: () => apiClient.get<TenantSubscription[]>("/api/v1/control-plane/subscriptions"),
  createSubscription: (body: { tenantId: string; planId: string; billingCycle: string; seatQuantity: number; trialDays?: number }) => apiClient.post<TenantSubscription>("/api/v1/control-plane/subscriptions", body),
  changeSubscriptionPlan: (subscriptionId: string, body: { planId: string; billingCycle: string; effectiveMode: string; reason: string }) => apiClient.patch<TenantSubscription>(`/api/v1/control-plane/subscriptions/${subscriptionId}/change-plan`, body),
  changeSubscriptionSeats: (subscriptionId: string, seatQuantity: number, reason: string) => apiClient.patch<TenantSubscription>(`/api/v1/control-plane/subscriptions/${subscriptionId}/seats`, { seatQuantity, reason }),
  cancelSubscription: (subscriptionId: string, immediate: boolean, reason: string) => apiClient.patch<TenantSubscription>(`/api/v1/control-plane/subscriptions/${subscriptionId}/cancel`, { immediate, reason }),
  ...
  markInvoicePaid: (invoiceId: string, paymentReference: string, reason: string) => apiClient.post<BillingInvoice>(`/api/v1/control-plane/billing/invoices/${invoiceId}/mark-paid`, { paymentReference, reason }),

  organizations: (tenantId: string) => apiClient.get<ManagedOrganization[]>(`/api/v1/control-plane/tenants/${tenantId}/organizations`),
  createOrganization: (tenantId: string, body: { name: string; description?: string }) => apiClient.post<ManagedOrganization>(`/api/v1/control-plane/tenants/${tenantId}/organizations`, body),
  updateOrganization: (tenantId: string, organizationId: string, body: { name: string; description?: string }) => apiClient.put<ManagedOrganization>(`/api/v1/control-plane/tenants/${tenantId}/organizations/${organizationId}`, body),
  changeOrganizationStatus: (tenantId: string, organizationId: string, status: string, reason: string) => apiClient.patch<ManagedOrganization>(`/api/v1/control-plane/tenants/${tenantId}/organizations/${organizationId}/status`, { status, reason }),
  ...
  createMembership: (tenantId: string, organizationId: string, body: { email: string; displayName?: string; roleCode: string }) => apiClient.post<ManagedMembership>(`/api/v1/control-plane/tenants/${tenantId}/organizations/${organizationId}/memberships`, body),
  updateMembership: (tenantId: string, organizationId: string, membershipId: string, body: { status: string; roleCode: string; reason: string }) => apiClient.patch<ManagedMembership>(`/api/v1/control-plane/tenants/${tenantId}/organizations/${organizationId}/memberships/${membershipId}`, body),
};