...
class CustomerMasterHttpIntegrationTest {
    ...
    @Test
    void readsDefaultEnterpriseProfileAndEnforcesTenantIsolation() throws Exception {
        Fixture a = fixture("master-a");
        Fixture b = fixture("master-b");
        UUID accountId = account(a, "Acme Arabia");
        ...
        mockMvc.perform(get("/api/v1/crm/accounts/{id}/master", accountId)
                        .with(authentication(auth(b))))
                .andExpect(status().isNotFound());
    }

    @Test
    void updatesEnterpriseIdentityClassificationRiskAndCredit() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/crm/accounts/{id}/master", accountId)
                        ...
                        .content("""
                                {
                                  "expectedVersion":0,
                                  "legalName":"SNAD Customer Company",
                                  "tradingName":"SNAD Customer",
                                  "registrationNumber":"CR-101010",
                                  "taxNumber":"VAT-3100000000",
                                  "industryCode":"SOFTWARE",
                                  "customerSegment":"ENTERPRISE",
                                  "customerTier":"STRATEGIC",
                                  "website":"https://customer.example",
                                  "primaryEmail":"finance@customer.example",
        ...
        Integer auditCount = jdbc.queryForObject(
                "SELECT COUNT(*) FROM audit_events WHERE tenant_id=:tenantId AND entity_id=:accountId",
    ...
    @Test
    void managesAddressesWithoutLeakingAcrossTenants() throws Exception {
        Fixture fixture = fixture("master-address");
        UUID accountId = account(fixture, "Addressed Customer");
        String body = mockMvc.perform(post("/api/v1/crm/accounts/{id}/addresses", accountId)
                        .with(authentication(auth(fixture)))
                        .contentType("application/json")
                        .content("""
                ...
                .andExpect(jsonPath("$.primaryAddress").value(true))
                .andReturn().getResponse().getContentAsString();
        UUID addressId = UUID.fromString(body.replaceAll(".*\\\"id\\\":\\\"([^\\\"]+)\\\".*", "$1"));

        mockMvc.perform(get("/api/v1/crm/accounts/{id}/addresses", accountId)
                        .with(authentication(auth(fixture))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].city").value("Riyadh"));
    ...
    @Test
    void preventsDuplicateEnterpriseIdentifierInsideTenant() throws Exception {
        Fixture fixture = fixture("master-id");
        UUID first = account(fixture, "First Customer");
        UUID second = account(fixture, "Second Customer");
        String identifier = "{\"identifierType\":\"COMMERCIAL_REGISTRATION\",\"identifierValue\":\"1010-2020\",\"issuerCountryCode\":\"SA\",\"primaryIdentifier\":true,\"verified\":true}";

        mockMvc.perform(post("/api/v1/crm/accounts/{id}/identifiers", first)
                        .with(authentication(auth(fixture))).contentType("application/json").content(identifier))
                .andExpect(status().isCreated());
    ...
    @Test
    void detectsDuplicateCandidatesUsingEnterpriseIdentity() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/crm/accounts/{id}/duplicates", source)
                        .with(authentication(auth(fixture))))
                .andExpect(status().isOk())
    ...
    @Test
    void mergesCustomerRecordsTransactionallyAndRecordsGovernanceHistory() throws Exception {
    ...
    private Fixture fixture(String key) {
        ...
        jdbc.update("INSERT INTO tenants (id,name,subdomain,status,created_at,updated_at) VALUES (:id,:name,:subdomain,'ACTIVE',:now,:now)",
                p().addValue("id", tenantId).addValue("name", key).addValue("subdomain", key + "-" + tenantId.toString().substring(0, 8)).addValue("now", now));
