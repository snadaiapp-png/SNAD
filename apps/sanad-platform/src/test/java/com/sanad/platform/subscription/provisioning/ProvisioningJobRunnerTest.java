 ...
 * to ACTIVE via a lifecycle transition.
 */
...
class ProvisioningJobRunnerTest {
    ...
    @BeforeEach
    void setUp() {
        runner = new ProvisioningJobRunner(jdbc);
    }
    ...
    @Test
    @DisplayName("success: all steps recorded, job SUCCEEDED, subscription set ACTIVE")
    void successPath() {
        jobRow("PENDING", 0);
        noCompletedSteps();
        ...
        verify(jdbc).update(contains("UPDATE provisioning_jobs SET status = 'SUCCEEDED'"),
                eq(JOB_ID));
        verify(jdbc).update(contains("UPDATE tenant_subscriptions SET status = 'ACTIVE'"),
                eq(SUBSCRIPTION_ID));
    }
