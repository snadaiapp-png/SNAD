...
/**
 * G7 correctness regression tests added after the repository-wide forensic review.
 *
 * These tests protect the two zero-data-loss invariants that were missing from the
 * original acceptance gate:
 *  - pull pagination is driven by a globally monotonic change id, never row-local
 *    sync_version values that can repeat across records;
 *  - push UPDATE binds scalar JDBC parameters in canonical CRM schema order.
 */
class G7CorrectnessHardeningTest {
    ...
    void pullUsesGlobalChangeIdSoEqualEntityVersionsCannotBeSkipped() {
        ...
        when(jdbc.queryForList(anyString(), any(Object[].class))).thenAnswer(invocation -> {
            ...
            if (!sql.contains("mobile_change_log")) {
                return List.of();
            }

            long afterChangeId = ((Number) params[2]).longValue();
            if (afterChangeId == 0L) {
                // Three different rows all have entity_version=1. A page size of 2
                // must return the first two while preserving the third for page 2.
                return List.of(
                    change(101L, ENTITY_1, 1L, "Acme"),
                    change(102L, ENTITY_2, 1L, "Beta"),
                    change(103L, ENTITY_3, 1L, "Gamma")
                );
            }
            if (afterChangeId == 102L) {
                return List.of(change(103L, ENTITY_3, 1L, "Gamma"));
            }
            return List.of();
        });
        ...
        assertEquals(List.of(ENTITY_1.toString(), ENTITY_2.toString()),
            first.entities().stream().map(DeltaSyncResponse.EntityDelta::entityId).toList());
        assertEquals(102L, decode(first.nextCursor()));

        DeltaSyncResponse second = service.pull(
            TENANT, DEVICE, new DeltaSyncRequest("account", first.nextCursor(), 2));
        ...
        assertEquals(1L, second.entities().get(0).version(),
            "entity version remains independent from the global change cursor");
    ...
    void pushUpdateFlattensJdbcParametersAndMapsLegacyAccountAliasToCanonicalColumn() {
        ...
        PushSyncRequest.MutationEnvelope mutation = new PushSyncRequest.MutationEnvelope(
            "g7-update-flat-1", "account", ENTITY_1.toString(), "UPDATE", 5L, payload, null);
        ...
        assertTrue(index >= 0, "canonical crm_accounts UPDATE was not executed");

        String sql = sqlCaptor.getAllValues().get(index);
        Object[] args = argsCaptor.getAllValues().get(index);
        assertTrue(sql.contains("display_name = ?"), "legacy mobile 'name' must map to CRM display_name");
        assertFalse(args[0] instanceof Object[], "JDBC arguments must be flat scalars, never a nested Object[]");
    ...
    void pushNumericFieldsRemainNumericInsteadOfBeingStringified() {
        ...
        PushSyncRequest.MutationEnvelope mutation = new PushSyncRequest.MutationEnvelope(
            "g7-update-number-1", "opportunity", ENTITY_1.toString(), "UPDATE", 7L, payload, null);
        ...
        for (int i = 0; i < sqlCaptor.getAllValues().size(); i++) {
            if (sqlCaptor.getAllValues().get(i).startsWith("UPDATE crm_opportunities")) {
                ...
                assertTrue(value instanceof Number, "numeric CRM fields must bind as Number, got " + value.getClass());
                return;
            }
        }
