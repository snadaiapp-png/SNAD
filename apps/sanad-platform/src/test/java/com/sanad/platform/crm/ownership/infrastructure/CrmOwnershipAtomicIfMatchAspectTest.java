...
class CrmOwnershipAtomicIfMatchAspectTest {
    ...
    void resolvesEveryGovernedOwnershipMutationToItsDatabaseRow() {
        ...
        assertTarget("DELETE", "/api/v2/crm/territories/" + territory + "/assignments/" + assignment,
                "crm_territory_assignments", "territory-assignment", assignment);
    ...
    @Test
    void ignoresReadsCreatesAndUnversionedCommands() {
        assertThat(resolve("GET", "/api/v2/crm/teams/" + UUID.randomUUID())).isNull();
        assertThat(resolve("POST", "/api/v2/crm/teams")).isNull();
        assertThat(resolve("POST", "/api/v2/crm/queues/" + UUID.randomUUID()
                + "/items/" + UUID.randomUUID() + "/claim")).isNull();
    }

    @Test
    void timestampVersionUsesPostgresqlMicrosecondPrecision() {
    ...
    private CrmOwnershipAtomicIfMatchAspect.LockTarget resolve(String method, String path) {
        return CrmOwnershipAtomicIfMatchAspect.resolveTarget(
