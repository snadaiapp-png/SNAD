...
public class JdbcActivityRepository implements ActivityRepository {
    ...
    public JdbcActivityRepository(NamedParameterJdbcTemplate jdbc) { this.jdbc = jdbc; }
    public ActivityRecord findById(UUID t, UUID id) {
        try { return mapRow(jdbc.queryForMap("SELECT * FROM crm_activities WHERE tenant_id=:t AND id=:id", new MapSqlParameterSource().addValue("t",t).addValue("id",id))); }
        catch (org.springframework.dao.EmptyResultDataAccessException e) { throw new CrmContractException(CrmErrorCode.CRM_ACTIVITY_NOT_FOUND); }
    }
    public List<ActivityRecord> findAll(UUID t, int limit, String relatedType, UUID relatedId, String status) {
        ...
        if (relatedType != null) { sql.append(" AND related_type=:relatedType"); params.addValue("relatedType", relatedType.toUpperCase()); }
        if (relatedId != null) { sql.append(" AND related_id=:relatedId"); params.addValue("relatedId", relatedId); }
        if (status != null) { sql.append(" AND status=:status"); params.addValue("status", status.toUpperCase()); }
    ...
    public ActivityRecord create(UUID t, UUID actorId, CreateActivityCommand cmd) {
        ...
        jdbc.update("INSERT INTO crm_activities (id,tenant_id,version,activity_type,subject,body,related_type,related_id,owner_user_id,status,priority,start_at,due_at,created_by,updated_by,created_at,updated_at) VALUES (:id,:t,0,:type,:subject,:body,:relatedType,:relatedId,:ownerUserId,'OPEN',:priority,:startAt,:dueAt,:actorId,:actorId,:now,:now)",
                new MapSqlParameterSource().addValue("id",id).addValue("t",t).addValue("type",cmd.activityType().toUpperCase()).addValue("subject",cmd.subject()).addValue("body",cmd.body())
                .addValue("relatedType",cmd.relatedType()==null?null:cmd.relatedType().toUpperCase()).addValue("relatedId",cmd.relatedId())
                .addValue("ownerUserId",cmd.ownerUserId()).addValue("priority",cmd.priority()==null?50:cmd.priority())
                .addValue("startAt",cmd.startAt()).addValue("dueAt",cmd.dueAt()).addValue("actorId",actorId).addValue("now",Timestamp.from(now)));
    ...
    public ActivityRecord update(UUID t, UUID actorId, UUID id, UpdateActivityCommand cmd, long expectedVersion) {
        ...
        if (cmd.subject() != null) { sql.append(",subject=:subject"); params.addValue("subject", cmd.subject()); }
        if (cmd.body() != null) { sql.append(",body=:body"); params.addValue("body", cmd.body()); }
        if (cmd.priority() != null) { sql.append(",priority=:priority"); params.addValue("priority", cmd.priority()); }
        if (cmd.startAt() != null) { sql.append(",start_at=:startAt"); params.addValue("startAt", cmd.startAt()); }
        if (cmd.dueAt() != null) { sql.append(",due_at=:dueAt"); params.addValue("dueAt", cmd.dueAt()); }
    ...
    public ActivityRecord complete(UUID t, UUID actorId, UUID id, String result, long expectedVersion) {
        int updated = jdbc.update("UPDATE crm_activities SET status='COMPLETED',result=:result,completed_at=:now,version=version+1,updated_by=:actorId,updated_at=:now WHERE tenant_id=:t AND id=:id AND version=:expectedVersion",
                new MapSqlParameterSource().addValue("t",t).addValue("id",id).addValue("expectedVersion",expectedVersion).addValue("result",result).addValue("actorId",actorId).addValue("now",Timestamp.from(Instant.now())));
        if (updated == 0) throw new CrmContractException(CrmErrorCode.CRM_CONCURRENCY_CONFLICT);
        return findById(t, id);
    }
