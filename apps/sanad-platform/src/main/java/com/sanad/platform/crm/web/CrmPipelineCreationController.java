...
/** Governed CRM v2 pipeline creation with idempotent create semantics. */
@RestController
@RequestMapping("/api/v2/crm")
public class CrmPipelineCreationController {
    private final CrmService crm;
    private final CrmDtoMapper mapper;
    private final CrmIdempotencyHttpSupport idempotency;

    public CrmPipelineCreationController(
            CrmService crm,
            CrmDtoMapper mapper,
    ...
    public ResponseEntity<SingleResponse<PipelineResponse>> createPipeline(
        ...
        try {
            PipelineResponse response = mapper.toPipelineResponse(crm.createPipeline(auth, body), List.of());
