 ...
 * development. Automated performance tests authenticate with a real JWT issued
 * by the ordinary login flow against an isolated PostgreSQL CI database.</p>
 *
 * <p>Two beans are provided:</p>
 * <ul>
 *   <li><b>CredentialBootstrapService</b> — required by the internal
 *       control-plane bootstrap controller.</li>
 *   <li><b>ApplicationRunner</b> — idempotently seeds the perf-test tenant,
 *       the perf-admin user, its ADMIN role with every active capability,
 *       organization membership, and the CRM fixtures referenced by k6.</li>
 * </ul>
 */
@Configuration
@Profile("perf-test")
public class PerfTestBootstrapConfig {
    ...
    private void seedEnvironment(JdbcTemplate jdbc, String passwordHash, String adminEmail) {
        ...
        jdbc.update("""
                ...
                SELECT gen_random_uuid(), ?, ?, cap.id, CURRENT_TIMESTAMP
