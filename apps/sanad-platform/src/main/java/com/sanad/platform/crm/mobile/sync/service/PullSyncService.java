...
public class PullSyncService {
    ...
    private static final Set<String> ENTITY_TYPES = MobileSyncSchema.ENTITY_TABLES.keySet();
    ...
    public DeltaSyncResponse pull(UUID tenantId, UUID deviceId, DeltaSyncRequest request) {
        ...
        if (!ENTITY_TYPES.contains(entityType)) throw new IllegalArgumentException("Unknown entity type: " + entityType);
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList("""
            ...
            WHERE tenant_id = ? AND entity_type = ? AND change_id > ?
        ...
        for (int i = 0; i < resultSize; i++) {
            ...
            JsonNode canonicalPayload = parsePayload(row.get("payload"));
            JsonNode mobilePayload = "DELETE".equals(operation)
                ? canonicalPayload
                : MobileSyncSchema.toMobilePayload(entityType, canonicalPayload, objectMapper);

            entities.add(new EntityDelta(entityId.toString(), operation, entityVersion, mobilePayload, changedAt));
            nextChangeId = changeId;
        }

        String nextCursor = resultSize == 0 ? request.cursor() : encodeCursor(nextChangeId);
        if (nextCursor == null) nextCursor = encodeCursor(sinceChangeId);
        ...
        return new DeltaSyncResponse(entityType, nextCursor, resultSize, entities, Instant.now(), hasMore);
    }

    private JsonNode parsePayload(Object payload) {
        if (payload == null) return NullNode.getInstance();
        try { return objectMapper.readTree(payload.toString()); }
        catch (Exception e) { throw new IllegalStateException("Invalid JSON payload in mobile_change_log", e); }
    }
    ...
    private void persistDeviceCursor(UUID tenantId, UUID deviceId, String entityType, String cursor, int entityCount) {
        ...
        """, tenantId, deviceId, entityType, cursor, cursorHash, entityCount, tenantId, deviceId);
        jdbcTemplate.update("""
            UPDATE mobile_device_registry SET last_sync_at = NOW(), updated_at = NOW()
            WHERE tenant_id = ? AND device_id = ? AND is_active = TRUE
        """, tenantId, deviceId);
    }
    ...
    private long decodeCursor(String cursor) {
        ...
        try {
            ...
            if (value < 0) throw new IllegalArgumentException("Negative cursor");
            return value;
        } catch (Exception e) { throw new IllegalArgumentException("Invalid sync cursor", e); }
    }

    private String encodeCursor(long changeId) {
        return Base64.getUrlEncoder().withoutPadding().encodeToString(Long.toString(changeId).getBytes(StandardCharsets.UTF_8));
    }
