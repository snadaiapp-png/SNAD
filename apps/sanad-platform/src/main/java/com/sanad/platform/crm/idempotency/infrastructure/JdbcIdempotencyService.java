package com.sanad.platform.crm.idempotency.infrastructure;
 ...
 * {@code crm_idempotency_records} table.
 */
public class JdbcIdempotencyService implements IdempotencyService {
    ...
    @Override
    public Replay begin(UUID tenantId, UUID principalId, String endpoint,
    ...
    public void complete(UUID operationId, int responseStatus, String responseBodyJson,
                         String responseHeadersJson, String contentType) {
        jdbc.update(
                "UPDATE crm_idempotency_records " +
                        "SET response_status = :status, response_body_json = :body, " +
                        "response_headers_json = :headers, content_type = :ct WHERE id = :id",
                new MapSqlParameterSource()
                        .addValue("id", operationId)
