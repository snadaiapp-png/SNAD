...
public interface LeadRepository {
    ...
    LeadRecord changeStatus(UUID tenantId, UUID actorId, UUID leadId, String status, String reason,
                            long expectedVersion);
    LeadConversionRecord convert(UUID tenantId, UUID actorId, UUID leadId,
                                 ConvertLeadCommand command, long expectedVersion);
    ...
    record ConvertLeadCommand(String accountName, Boolean createOpportunity, UUID pipelineId,
            ...
            java.time.LocalDate expectedCloseDate, UUID accountId, UUID contactId,
            UUID opportunityId) {}

    record LeadConversionRecord(LeadRecord lead, UUID accountId, UUID contactId,
                                UUID opportunityId, boolean idempotent) {}
}
