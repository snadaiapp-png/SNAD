...
public class JdbcLeadRepository implements LeadRepository {
    ...
    public JdbcLeadRepository(NamedParameterJdbcTemplate jdbc) { this.jdbc = jdbc; }

    public LeadRecord findById(UUID tenantId, UUID leadId) {
        try { return mapRow(jdbc.queryForMap("SELECT * FROM crm_leads WHERE tenant_id=:tenantId AND id=:id", p(tenantId, leadId))); }
        catch (org.springframework.dao.EmptyResultDataAccessException e) { throw new CrmContractException(CrmErrorCode.CRM_LEAD_NOT_FOUND); }
    }
    public List<LeadRecord> findAll(UUID tenantId, int limit, String status) {
        ...
        if (status != null && !status.isBlank()) { sql.append(" AND status=:status"); params.addValue("status", status.toUpperCase()); }
    ...
    public LeadRecord create(UUID tenantId, UUID actorId, CreateLeadCommand cmd) {
        ...
        jdbc.update("INSERT INTO crm_leads (id,tenant_id,version,display_name,normalized_name,company_name,email,normalized_email,phone,source,status,owner_user_id,score,created_by,updated_by,created_at,updated_at) VALUES (:id,:tenantId,0,:displayName,LOWER(:displayName),:companyName,:email,LOWER(:email),:phone,:source,'NEW',:ownerUserId,:score,:actorId,:actorId,:now,:now)",
                new MapSqlParameterSource().addValue("id",id).addValue("tenantId",tenantId).addValue("displayName",cmd.displayName())
                .addValue("companyName",cmd.companyName()).addValue("email",cmd.email()).addValue("phone",cmd.phone())
                .addValue("source",cmd.source()).addValue("ownerUserId",cmd.ownerUserId()).addValue("score",cmd.score())
    ...
    public LeadRecord changeStatus(UUID tenantId, UUID actorId, UUID leadId, String status, String reason, long expectedVersion) {
        int updated = jdbc.update("UPDATE crm_leads SET status=:status,updated_by=:actorId,updated_at=:now,version=version+1 WHERE tenant_id=:tenantId AND id=:id AND version=:expectedVersion",
                new MapSqlParameterSource().addValue("tenantId",tenantId).addValue("id",leadId).addValue("expectedVersion",expectedVersion)
                .addValue("status",status.toUpperCase()).addValue("actorId",actorId).addValue("now",Timestamp.from(Instant.now())));
    ...
    public LeadConversionRecord convert(UUID tenantId, UUID actorId, UUID leadId, ConvertLeadCommand cmd, long expectedVersion) {
        ...
        if ("CONVERTED".equals(lead.status())) return new LeadConversionRecord(lead, lead.convertedAccountId(), lead.convertedContactId(), lead.convertedOpportunityId(), true);
        int updated = jdbc.update("UPDATE crm_leads SET status='CONVERTED',updated_by=:actorId,updated_at=:now,version=version+1 WHERE tenant_id=:tenantId AND id=:id AND version=:expectedVersion",
                new MapSqlParameterSource().addValue("tenantId",tenantId).addValue("id",leadId).addValue("expectedVersion",expectedVersion).addValue("actorId",actorId).addValue("now",Timestamp.from(Instant.now())));
        if (updated == 0) throw new CrmContractException(CrmErrorCode.CRM_CONCURRENCY_CONFLICT);
        LeadRecord updatedLead = findById(tenantId, leadId);
        return new LeadConversionRecord(updatedLead, null, null, null, false);
    }
    private LeadRecord mapRow(Map<String,Object> r) {
        return new LeadRecord((UUID)r.get("id"),asLong(r.get("version")),(String)r.get("display_name"),(String)r.get("company_name"),
                (String)r.get("email"),(String)r.get("phone"),(String)r.get("source"),(String)r.get("status"),
                (UUID)r.get("owner_user_id"),(BigDecimal)r.get("score"),(UUID)r.get("converted_account_id"),
                (UUID)r.get("converted_contact_id"),(UUID)r.get("converted_opportunity_id"),
                asInstant(r.get("created_at")),asInstant(r.get("updated_at")));
    }
    private MapSqlParameterSource p(UUID tenantId, UUID id) { return new MapSqlParameterSource().addValue("tenantId",tenantId).addValue("id",id); }
