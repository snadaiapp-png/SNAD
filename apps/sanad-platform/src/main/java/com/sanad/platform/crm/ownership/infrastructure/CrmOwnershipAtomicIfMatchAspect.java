...
/**
 * PostgreSQL concurrency authority for CRM-008 If-Match mutations.
 *
 * <p>The target row is locked and its ETag is validated inside a transaction
 * that remains active through the controller and application mutation. Two
 * requests carrying the same ETag cannot both mutate the row: the second waits
 * for the first commit, observes the new timestamp and fails with HTTP 412.</p>
 */
@Aspect
@Component
public class CrmOwnershipAtomicIfMatchAspect {
    ...
    public Object enforceAtomicIfMatch(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (tenantId == null) {
            // Authentication and capability aspects remain authoritative.
        ...
        try {
            return transactions.execute(status -> {
                ...
                if (updatedAt == null) return proceed(joinPoint);
    ...
    static LockTarget resolveTarget(HttpServletRequest request) {
        ...
        if (target != null && ("PATCH".equals(method) || "DELETE".equals(method))) return target;
