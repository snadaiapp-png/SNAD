...
-- IMPORTANT: bootstrap runs before FORCE RLS so Flyway does not depend on app.tenant_id.
...
-- This is intentionally before FORCE RLS: the Flyway role must not need app.tenant_id
-- while seeding tenant rows from their canonical CRM tables.
...
-- Runtime protection starts only after bootstrap is complete.
