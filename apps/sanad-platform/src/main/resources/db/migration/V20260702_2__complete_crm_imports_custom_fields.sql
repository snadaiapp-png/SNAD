...
-- Adds durable import storage, row-level import evidence, and typed custom-field values.
...
ALTER TABLE crm_import_jobs ADD CONSTRAINT ck_crm_import_jobs_attempts CHECK (attempt_count >= 0);

CREATE INDEX idx_crm_import_jobs_worker
    ON crm_import_jobs (status, created_at, tenant_id);
...
CREATE TABLE crm_import_errors (
    ...
    CONSTRAINT fk_crm_import_errors_job_same_tenant
        FOREIGN KEY (tenant_id, import_job_id) REFERENCES crm_import_jobs (tenant_id, id),
    CONSTRAINT ck_crm_import_errors_row CHECK (row_number >= 1)
);
...
FROM (
    VALUES
        ...
        ('CRM.IMPORT.WRITE', 'Run CRM Imports', 'Upload, validate, execute, and cancel tenant CRM imports')
