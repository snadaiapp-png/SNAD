...
## HRM-G1 T10 Final Closure Production Alignment Authority — 2026-09-19

- Certified current main SHA: `f17a1fd87c0a13528f52b48eae0168c9f5c80d31`
- HRM-G1 T10 squash merge SHA retained in current main ancestry: `89d7226e31a10a13bac8fb1d055a63ecf83eb731`
- T10 Post-Merge Main Verification #1077: `SUCCESS`
- T10 main CI #3888: `SUCCESS`
- Current-main Post-Merge Main Verification #1078: `SUCCESS`
- Current-main Web CI #4886: `SUCCESS`
- Current-main Stage 07 Artifact Provenance #4016: `SUCCESS`
- Current-main CRM Deployment Readiness #3078: `SUCCESS`
- Workflow Production 3-User Final Gate #19: `FAIL_CLOSED` before journey execution because the live Render backend image source is older than current main.
- Observed live Render production image before alignment: `ghcr.io/snadaiapp-png/snad-backend:93b66b0c0b5dc8def2e297052147e9b506cbe7ed`
- Application source drift from T10 merge to current main: `NONE` under `apps/sanad-platform/**` and `apps/web/**` as enforced by the final gate baseline guard.
- Runtime application behavior change in this authorization PR: `NONE`
- Database migration content change in this authorization PR: `NONE`
- Security/RBAC semantic change in this authorization PR: `NONE`
- Independent human review with repository write access: `REQUIRED`
...
This authorization is strictly limited to the final HRM-G1 T10 / Workflow Y2 production-alignment closure. It exists to create a protected release-control main commit under `apps/sanad-platform/**`, which triggers immutable exact-SHA image publication without changing runtime semantics.

The protected squash merge MUST contain the exact immutable commit-message marker `PRODUCTION-RELEASE-AUTHORIZED`. Without that marker, the production orchestrator must fail closed.

Only the canonical path is authorized:

`Publish Render Backend Image` → immutable exact-main image → `Workflow Y2 Production Release Orchestrator` → `production-release.yml` with `rollback_on_failure=true` → exact-image Render verification → readiness → Flyway/runtime invariants → security/SCP/Vercel evidence → rerun of the exact-main Workflow Production 3-User Final Gate.

No manual Render deployment, direct production DDL, Flyway history mutation, `flyway repair`, out-of-order migration execution, force push, branch-protection bypass, credential mutation, catalog fabrication, or test/gate weakening is authorized.
