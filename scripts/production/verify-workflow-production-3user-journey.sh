...
# 5) Do not require pre-existing workflow data. A fresh Tenant B is valid.
# The validate boundary is proven below against the definition created by this final gate.
...
record validateBoundary PASS 'WORKFLOW.VALIDATE boundary verified on gate-owned definition; no pre-existing workflow data required'
