...
const authProbe=await fetch(`${base}/api/platform/api/v1/auth/login`,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify({email,password,tenantId})});
if(!authProbe.ok) throw new Error(`QA auth probe HTTP ${authProbe.status}`);
const authBody=await authProbe.json();
if(authBody?.user?.tenantId!==tenantId) throw new Error("QA auth probe tenant binding mismatch");
