...
RULES = [
    ...
    ("prod-password-yaml", r'(?i)\b[A-Z0-9_]*PASSWORD\s*:\s*["\']?([^$<{"\'\s][^#\s"\']{7,})["\']?', "CRITICAL", "Hardcoded password in production configuration"),
    ("prod-password-shell", r'(?i)\b[A-Z0-9_]*PASSWORD\s*=\s*["\']?([^$<{"\'\s][^#\s"\']{7,})["\']?', "CRITICAL", "Hardcoded password assignment in production configuration"),
...
SKIP_DIRS = {
    ...
    # Disabled workflows remain part of the current repository tree and are
    # intentionally scanned for secrets. Disabling execution is not a valid
    # reason to retain plaintext credentials or credential-reset material.
}
