...
| 5 | **Event-Driven Architecture** — async integration via events | PLANNED — not yet implemented (no event broker in current stack) |
| 6 | **Workflow-First** — business processes are first-class services | PLANNED — workflow services not yet built |
| 7 | **AI-First** — AI is a peer of CRUD, not a side feature | PLANNED — no AI agents yet (LangChain4j + Qdrant pending) |
| 8 | **Security by Design** — defense in depth, least privilege | Every endpoint declares `@RequireCapability` |
| 9 | **Observability** — structured logs, metrics, traces | PARTIAL — health/readiness probes; structured logging pending (DEFECT-026) |
| 10 | **Horizontal Scalability** — stateless services, distributed rate limits | PARTIAL — single-instance deployment; rate limiting is in-memory only (DEFECT-015 open) |
...
- `tenantId` is extracted from the JWT (SecurityContext), never from the request body or query parameters. (EXEC-PROMPT-010R: controllers must not accept tenantId as a request parameter — see PR #83 for the corrected pattern.)
...
- **Branch protection on `main` is REQUIRED** (require PR, require approvals, require status checks, block force-push). Status as of 2026-06-25: NOT YET CONFIGURED — owner action required.
