...
2. Local `expo prebuild` CLI is blocked by an SDK-52 packaging quirk on Node (packages ship Metro-style extensionless imports); the prebuild+manifest+APK path is therefore the CI job in `g8-android-native-validation.yml` (fresh runner, canonical toolchain), while the config-plugin logic itself is validated locally and idempotency is asserted in CI by a second prebuild.
