...
class WorkflowFinalClosureUnifiedContractTest(unittest.TestCase):
    ...
    def test_authenticated_desktop_mobile_visual_contract(self):
        ...
        self.assertIn('/?tenantId=${encodeURIComponent(tenantId)}', self.visual)
        self.assertNotIn('page.route("**/api/platform/api/v1/auth/login"', self.visual)
        self.assertIn('form button[type="submit"]', self.visual)
        self.assertIn("for(let attempt=1;attempt<=6&&!loginRequest;attempt++)", self.visual)
        self.assertIn("page.waitForRequest(request=>", self.visual)
        self.assertIn("loginRequestBody.tenantId!==tenantId", self.visual)
