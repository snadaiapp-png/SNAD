...
class WorkflowFinalClosureUnifiedContractTest(unittest.TestCase):
    ...
    def test_empty_dedicated_qa_tenant_is_valid_precondition(self):
        self.assertIn("Workflow definitions response must be an array", self.journey)
        self.assertIn("Definitions endpoint authorized; lifecycle validation is proven on the controlled definition created below", self.journey)
    ...
    def test_authenticated_desktop_mobile_visual_contract(self):
        ...
        self.assertIn('page.route("**/api/platform/api/v1/auth/login"', self.visual)
        self.assertIn("JSON.stringify({...requestBody,tenantId})", self.visual)
        self.assertIn('form button[type="submit"]', self.visual)
        self.assertIn('page.locator("form").waitFor({state:"visible",timeout:30000})', self.visual)
        self.assertIn("page.waitForFunction(()=>", self.visual)
        self.assertIn('key.startsWith("__reactProps$")||key.startsWith("__reactFiber$")', self.visual)
        self.assertLess(self.visual.index("page.waitForFunction(()=>"), self.visual.index('page.locator("#login-email").fill(email)'))
        self.assertIn("page.waitForRequest(request=>", self.visual)
        self.assertIn("const loginResponse=await loginRequest.response()", self.visual)
        self.assertNotIn("page.waitForResponse(response=>", self.visual)
        self.assertIn("loginBody?.user?.tenantId!==tenantId", self.visual)
        self.assertIn("credentialRotationRequired===true", self.visual)
        self.assertNotIn('page.locator("#login-password").press("Enter")', self.visual)
