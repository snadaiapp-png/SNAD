...
class WorkflowFinalClosureUnifiedContractTest(unittest.TestCase):
    ...
    def test_authenticated_desktop_mobile_visual_contract(self):
        ...
        self.assertIn('fetch("/api/platform/api/v1/auth/login"', self.visual)
        ...
        self.assertIn('authBootstrap:"BROWSER_BFF_LOGIN_REFRESH"', self.visual)
        self.assertNotIn("page.waitForRequest(request=>", self.visual)
        self.assertNotIn("page.waitForResponse(response=>", self.visual)
        self.assertNotIn('form button[type="submit"]', self.visual)
        self.assertIn("loginResult.body?.user?.tenantId!==tenantId", self.visual)
